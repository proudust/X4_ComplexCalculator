...
namespace X4_ComplexCalculator.DB.X4DB
{
    ...
    public class Module : IComparable
    {
        ...
        public Module(string moduleID, string name, ModuleType moduleType, long maxWorkers, long workersCapacity, ModuleProduction[] buildMethods, Faction[] owners)
        {
            ModuleID = moduleID;
            Name = name;
    ...
    public static class ModuleTable
    {
        ...
        public static void Init()
        {
            ...
            X4Database.Instance.ExecQuery("SELECT ModuleID, ModuleTypeID, Name, MaxWorkers, WorkersCapacity, NoBlueprint FROM Module", (dr, args) =>
            {
                ...
                var moduleType = ModuleTypeTable.Get((string)dr["ModuleTypeID"]);
                ...
                {
                    var faction = FactionTable.Get((string)dr2["FactionID"]);
                    if (faction != null) moduleOwners.Add(faction);
                });

                var prod = (noBlueprint) ? Array.Empty<ModuleProduction>() : ModuleProductionTable.Get(id);
